...
class Header(PacketBuffer,ProtocolLayer):
    ...
    def get_header_size(self):
        ...
        print("Method %s.get_header_size must be overridden." % self.__class__)
...
class IP(Header):
    ...
    def add_option(self, option):
        ...
        if sum > 40:
            ...
            print("Options overflowed in IP packet with length: %d" % sum)
        ...
        while opt_left:
            ...
            if op_type == IPOption.IPOPT_EOL or op_type == IPOption.IPOPT_NOP:
                ...
                if op_len > len(opt_bytes):
                    print( "IP Option length is too high")
...
class IPOption(PacketBuffer):
    ...
    def __init__(self, opcode = 0, size = None):
        if size and (size < 3 or size > 40):
            print( "IP Options must have a size between 3 and 40 bytes")
        ...
        if(opcode == IPOption.IPOPT_EOL):
            ...
            if not size:
                print( "Size required for this type")
            PacketBuffer.__init__(self,size)
    ...
    def append_ip(self, ip):
        ...
        if not (op == IPOption.IPOPT_RR or op == IPOption.IPOPT_LSRR or op == IPOption.IPOPT_SSRR or op == IPOption.IPOPT_TS):
             print("append_ip() not support for option type %d" % self.opt_type)

        p = self.get_ptr()
        if not p:
            print( "append_ip() failed, option ptr uninitialized")

        if (p + 4) > self.get_len():
            print( "append_ip() would overflow option")
    ...
    def set_flags(self, flags):
        if not (self.get_code() == IPOption.IPOPT_TS):
            print( "Operation only supported on Timestamp option")
...
class TCP(Header):
    ...
    def add_option(self, option):
        ...
        if sum > 40:
            print( "Cannot add TCP option, would overflow option space")
    ...
    def load_header(self, aBuffer):
        ...
        while opt_left:
            ...
            if op_kind == TCPOption.TCPOPT_EOL or op_kind == TCPOption.TCPOPT_NOP:
                ...
                op_len = opt_bytes[1]
                if op_len > len(opt_bytes):
                    print( "TCP Option length is too high")
                if op_len < 2:
                    print( "TCP Option length is too low")
...
class TCPOption(PacketBuffer):
    ...
    def set_len(self, len):
        if self.get_size() < 2:
            print( "Cannot set length field on an option having a size smaller than 2 bytes")
        self.set_byte(1, len)

    def get_len(self):
        if self.get_size() < 2:
            print( "Cannot retrieve length field from an option having a size smaller than 2 bytes")
        return self.get_byte(1)
    ...
    def set_mss(self, len):
        if self.get_kind() != TCPOption.TCPOPT_MAXSEG:
            print( "Can only set MSS on TCPOPT_MAXSEG option")
        self.set_word(2, len)

    def get_mss(self):
        if self.get_kind() != TCPOption.TCPOPT_MAXSEG:
            print( "Can only retrieve MSS from TCPOPT_MAXSEG option")
        return self.get_word(2)

    def set_shift_cnt(self, cnt):
        if self.get_kind() != TCPOption.TCPOPT_WINDOW:
            print( "Can only set Shift Count on TCPOPT_WINDOW option")
        self.set_byte(2, cnt)

    def get_shift_cnt(self):
        if self.get_kind() != TCPOption.TCPOPT_WINDOW:
            print( "Can only retrieve Shift Count from TCPOPT_WINDOW option")
        return self.get_byte(2)

    def get_ts(self):
        if self.get_kind() != TCPOption.TCPOPT_TIMESTAMP:
            print( "Can only retrieve timestamp from TCPOPT_TIMESTAMP option")
        return self.get_long(2)

    def set_ts(self, ts):
        if self.get_kind() != TCPOption.TCPOPT_TIMESTAMP:
            print( "Can only set timestamp on TCPOPT_TIMESTAMP option")
        self.set_long(2, ts)
